...
function Sidebar() {
  ...
  // Check if current user session is superadmin (either gic_user or userRole)
  const isSuperadmin = currentUser.role === "superadmin" || localStorage.getItem("userRole") === "superadmin";
  ...
  return (
    <aside className="w-72 lg:w-64 h-screen overflow-y-auto bg-[#F8F9FA] border-r border-gray-200 px-4 py-4">
      ...
      <div className="space-y-2">
        {/* Company Subscription Plan card (Visible to regular admins/companies) */}
        {isAdmin && (
          <Link to="/company/subscription" className="flex items-center gap-3 px-3 py-3 rounded-lg hover:bg-gray-100">
